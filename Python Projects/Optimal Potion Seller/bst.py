...
__author__ = 'Brendon Taylor, modified by Alexey Ignatiev and Jackson Goerner, ' \
             'modified by (Zhi Yong Lee, Terenia Lee Leh Yi, Yu Wen Liew, Jeremy Koh Chi Minn)'
...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            :complexity: worst case: O(n) where n is the depth of the tree/subtree
        """
        # Checks if current.right is None
        if current.right is not None:
            # Directs current to current.right
            current = current.right
            # Now, directs current to current.left until it reaches a node where current.left is None
            while current.left is not None:
                current = current.left
            return current
        # Return None if current.right is None
        return None

    #get_minimal implementation
    def get_minimal(self, current: TreeNode) -> TreeNode:
        """
            ...
            :complexity: worst case: O(n) where n is the depth of the tree/subtree
        """
        # Keeps directing current to current.left until it reaches a node where current.left is None
